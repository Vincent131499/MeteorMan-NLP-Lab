...
class TransformerDSSM:
	def __init__ (self,
		...
		self.q_max_length = 0
		self.t_max_length = 0
		# self.model_save_name = './ModelMemory/model/transformerDSSM.ckpt'
		self.model_save_name = './ModelMemory/model-Sim/transformerDSSM'
		# self.model_save_checkpoint = './ModelMemory/model/checkpoint'
		self.model_save_checkpoint = './ModelMemory/model-Sim/checkpoint'
	...
	def train (self, gpu_num=1):
		...
		with tf.Session(graph=self.graph, config=config) as self.session:
			...
			for i in range(self.epoch_steps):
				...
				for j in tqdm.tqdm(range(self.q_size // sample_nums)):
					if self.is_sample:
